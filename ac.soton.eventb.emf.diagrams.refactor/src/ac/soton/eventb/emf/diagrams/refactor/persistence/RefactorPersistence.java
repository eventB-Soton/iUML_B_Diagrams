 ...
 *******************************************************************************/
package ac.soton.eventb.emf.diagrams.refactor.persistence;
...
public class RefactorPersistence {
	...
	public void checkPreState(EventBNamedCommentedComponentElement component) throws IOException {
		...
		if (preComponent==null || equivalenceMap==null){
			...
			//Alternative using refiner- Needs further work?
//			AbstractElementRefiner refiner = ElementRefinerRegistry.getRegistry().getRefiner(component);
//			Map<EObject,EObject> copier = refiner.cloneAsAlternativeRefinement(component, null, preComponentRes.getURI());
//			preComponent = (EventBNamedCommentedComponentElement) copier.get(component);
//			
//			preComponentRes.getContents().add(0, preComponent);
//			preComponentRes.save(Collections.EMPTY_MAP);
//			saveEquivalenceMap(res, copier);		
